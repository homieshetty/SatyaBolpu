...
export const getTags = async (req: Request, res: Response) => {
  try {
    ...
    const tagsData = await Tag.find({}, 'name')
    ...
    return res
      .status(500)
      .json({ msg: 'Internal server error while fetching tags' });
  }
};
...
export const addTag = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ msg: 'Internal sever error while adding tag.' });
  }
};
...
export const getPostTypes = async (req: Request, res: Response) => {
  try {
    ...
    const postTypesData = await PostType.find({}, 'name')
      .collation({ locale: 'en', strength: 2 })
      .sort({ [sortBy]: orderBy === 'asc' ? 1 : -1 })
      .lean();
    if (!postTypesData) {
    ...
    return res
      .status(500)
      .json({ msg: 'Internal server error while fetching post types.' });
  }
};
...
export const addPostType = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ msg: 'Internal server error while adding post type.' });
  }
};
...
export const getPostGroups = async (req: Request, res: Response) => {
  try {
    ...
    const postGroupsData = await PostGroup.find({}, 'name')
      .collation({ locale: 'en', strength: 2 })
      .sort({ [sortBy]: orderBy === 'asc' ? 1 : -1 })
      .lean();
    if (!postGroupsData) {
    ...
    return res
      .status(500)
      .json({ msg: 'Internal server error while fetching post groups.' });
  }
};
...
export const addPostGroup = async (req: Request, res: Response) => {
  try {
    ...
    const postGroup = await PostGroup.create({ name });
    return res.status(200).json({ name: postGroup.name });
  } catch (err: any) {
    console.error(`Error while adding post group. : ${err}`);
    return res
      .status(500)
      .json({ msg: 'Internal server error while adding post group.' });
  }
};
