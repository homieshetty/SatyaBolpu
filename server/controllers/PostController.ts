...
export const getPosts = async (req: Request, res: Response) => {
  try {
    ...
    const postTypes = req.query.postTypes
      ? req.query.postTypes.toString().split(',')
      : [];
    ...
    if (tags.length)
      filterOptions.tags = { $in: tags.map((tag) => new Types.ObjectId(tag)) };
    if (postTypes.length)
      filterOptions.postType = {
        $in: postTypes.map((pt) => new Types.ObjectId(pt)),
      };
    ...
    return res
      .status(500)
      .json({ msg: 'Internal Server error while fetching posts.' });
  }
};
...
export const getPostGroups = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ postGroups, total, totalPages: Math.ceil(total / limit) });
  } catch (err: any) {
    console.error('Error while fetching posts: ', err.message);
    return res
      .status(500)
...
export const getPost = async (req: Request, res: Response) => {
  try {
    ...
    const postRes = await Post.findOne({ _id: id })
      .populate('culture')
      .populate('postType')
      .populate('postGroup')
      .populate('tags', 'name')
    ...
    console.error(
      `Error while fetching post with id ${req.params.id}: `,
      err.message,
    );
    return res.status(500).json({
      msg: `Internal Server error while fetching post with id ${req.params.id}.`,
    });
  }
};
...
export const createDraft = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ msg: 'Internal Server error while creating post draft.' });
  }
};
...
export const uploadPost = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (exists) {
      return res
        .status(409)
        .json({ msg: `Post '${details.title}' already exists.` });
    }
    ...
    return res
      .status(500)
      .json({ msg: 'Internal Server error while uploading post.' });
  }
};
